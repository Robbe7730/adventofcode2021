...
for line in io.lines() do
  table.insert(numbers, reduce(json.decode(line)))
end

maxMagnitude = 0
for i, firstNum in ipairs(numbers) do
  for j, secondNum in ipairs(numbers) do
    if i ~= j then
      mag = magnitude(reduce({firstNum, secondNum}))
