...
)
{
    ...
    if (value == input) {
        is_marked[values_idx] = 0;
        atomic_sub(&row_col_marked[board * 10 + row], 1);
        atomic_sub(&row_col_marked[board * 10 + 5 + col], 1);
        has_won = 0;
    }
    ...
    if (has_won == 0 && is_marked[values_idx] != 1 && value != input) {
        atomic_add(result, value * input);
    }
