...
fun main() {
    ...
    while (line != null) {
        ...
        val origSize = boardCol.size
        for (i in origSize..(5*origSize - 1)) {
            boardCol.add(((boardCol[i % origSize] + (i / origSize) - 1) % 9)+1)
        }
        board.add(boardCol)
        line = readLine()
    }

    val origSize = board.size
    for (i in origSize..(5*origSize - 1)) {
        board.add(
            board[i % origSize].map { ((it+(i/origSize) - 1) % 9) + 1 }.toMutableList()
        )
    }
